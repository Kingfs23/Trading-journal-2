...
function mergeTradeData(existing, incoming) {
  ...
  return {
    ...
    sync_status: existing.sync_status === "synced" || incoming.sync_status === "synced" ? "synced" : "local",
  };
}
...
function mergeTrades(localRows, remoteRows) {
  ...
  [...remoteRows, ...localRows].forEach((row) => {
    ...
    const directKey = row.id || row.local_id || "";
    const signature = tradeSignature(normalized);
    const key = directKey || signature;
    const existingKey = directKey && merged.has(String(directKey))
      ? String(directKey)
      : signatureToKey.get(signature);
    const finalKey = existingKey || String(key);

    merged.set(finalKey, mergeTradeData(merged.get(finalKey), normalized));
    if (signature) signatureToKey.set(signature, finalKey);
  });
