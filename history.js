...
const COLUMN_MAP = {
  ...
  // pair column may be "pair" if you have it; otherwise it will show "—"
  pair: ["pair", "symbol", "market"],

  // bias: your schema includes htf_bias
...
function getClient() {
  ...
  for (const c of candidates) {
    if (c && typeof c.from === "function") return c; // already a client
  }

  // If window.supabase is the namespace (from CDN), not the client, build a client if keys are exposed
  if (window.supabase && typeof window.supabase.createClient === "function") {
    const url = window.SUPABASE_URL || window.supabaseUrl || window.SUPABASEPROJECTURL;
    const key = window.SUPABASE_ANON_KEY || window.SUPABASE_KEY || window.supabaseAnonKey;
    if (url && key) {
      const built = window.supabase.createClient(url, key);
      window.supabaseClient = built;
      return built;
    }
  }
...
async function fetchTrades() {
  ...
  if (!client) {
    historyContainer.innerHTML = `<div class="state">
      I couldn't find a valid Supabase <b>client</b> (something with <code>.from()</code>).<br><br>Fix it by setting in <b>supabase.js</b>:<br><code>window.supabaseClient = client;</code><br>or<br><code>window.sb = client;</code><br><br>Supported globals:
