...
const syncLocalBtn = el("syncLocalBtn");
...
function publicUrlForPath(path) {
  ...
  try {
    return window.sb?.storage?.from(IMAGE_BUCKET).getPublicUrl(path).data.publicUrl || null;
  } catch {
    return null;
  }
}
...
function normalizeTrade(row) {
  ...
  const monthKey = date && !Number.isNaN(date.getTime())
    ...
    : "Unknown";
  const beforeImageValue = pick(row, ["before_url", "before_path", "before_img", "before_local_url"]);
  const afterImageValue = pick(row, ["after_url", "after_path", "after_img", "after_local_url"]);
  ...
  return {
    ...
    before_image_path: imagePathFromValue(beforeImageValue),
    after_image_path: imagePathFromValue(afterImageValue),
    before_url: resolveImage(beforeImageValue),
    after_url: resolveImage(afterImageValue),
...
function bestImage(first, second) {
  if (first && !isDataImage(first)) return first;
  if (second && !isDataImage(second)) return second;
...
function mergeTradeData(existing, incoming) {
  ...
  return {
    ...
    before_image_path: existing.before_image_path || incoming.before_image_path,
    after_image_path: existing.after_image_path || incoming.after_image_path,
...
async function signedUrlForPath(path) {
  if (!path || !window.sb?.storage) return publicUrlForPath(path);
...
async function hydrateTradeImages(trades) {
  if (!window.sb?.storage) return trades;

  return Promise.all(trades.map(async (trade) => {
    const beforePath = trade.before_image_path || imagePathFromValue(trade.before_url);
    const afterPath = trade.after_image_path || imagePathFromValue(trade.after_url);

    return {
      ...trade,
      before_image_path: beforePath,
      after_image_path: afterPath,
      before_url: beforePath ? await signedUrlForPath(beforePath) : trade.before_url,
      after_url: afterPath ? await signedUrlForPath(afterPath) : trade.after_url,
    };
  }));
}
...
function localDataImage(row, side) {
  const main = row?.[`${side}_url`];
  const local = row?.[`${side}_local_url`];
  if (isDataImage(main)) return main;
  if (isDataImage(local)) return local;
  return "";
}

function localPortableImage(row, side) {
  const main = row?.[`${side}_url`];
  if (main && !isDataImage(main)) return main;
  return "";
}

async function uploadDataImage(dataUrl, row, side) {
  if (!dataUrl) return null;

  const blob = dataUrlToBlob(dataUrl);
  const datePart = safePathPart(row.date || row.dateRaw || new Date().toISOString().slice(0, 10));
  const pairPart = safePathPart(row.pair || row.symbol || "trade");
  const extension = blob.type === "image/png" ? "png" : "jpg";
  const filePath = `trades/${datePart}/${pairPart}-${side}-${safeId()}.${extension}`;
...
function buildSupabaseTrade(row, beforeImage, afterImage) {
  return {
    created_at: row.created_at || new Date().toISOString(),
    date: row.date || row.dateRaw || row.entry_date || null,
    pair: String(row.pair || row.symbol || row.market || "").toUpperCase(),
    risk: toNumber(row.risk),
    htf_bias: row.htf_bias || row.bias || row.direction || null,
    poi: row.poi || null,
    inducement: row.inducement || null,
    sl: row.sl || row.stop || row.stop_loss || null,
    tp: row.tp || row.take_profit || null,
    rr: row.rr || row.r || null,
    pnl: toNumber(row.pnl ?? row.profit ?? row.net ?? row.amount),
    result: row.result || row.outcome || row.status || null,
    notes: row.notes || row.comment || null,
    emotions: row.emotions || null,
    improve_win: row.improve_win || null,
    improve_loss: row.improve_loss || null,
    improve_be: row.improve_be || null,
    before_url: beforeImage || null,
    after_url: afterImage || null,
  };
}

function cleanedSyncedLocalRow(row, data, beforeImage, afterImage) {
  const cleaned = {
    ...row,
    ...(data || {}),
    before_url: beforeImage || row.before_url || null,
    after_url: afterImage || row.after_url || null,
    sync_status: "synced",
  };

  delete cleaned.before_local_url;
  delete cleaned.after_local_url;
  return cleaned;
}

function localRowHasLaptopOnlyImages(row) {
  return Boolean(
    localDataImage(row, "before") ||
    localDataImage(row, "after") ||
    row.before_local_url ||
    row.after_local_url
  );
}

function localRowNeedsRemoteSave(row) {
  if (row.import_id) return false;
  if (row.sync_status !== "synced") return true;
  return Boolean(
    (localDataImage(row, "before") && !localPortableImage(row, "before")) ||
    (localDataImage(row, "after") && !localPortableImage(row, "after"))
  );
}

let syncInProgress = false;

async function syncLocalTradesToSupabase({ manual = false } = {}) {
  if (syncInProgress || !window.sb?.from || !window.sb?.storage) return 0;

  const localRows = readLocalTrades();
  const candidateIndexes = localRows
    .map((row, index) => ({ row, index }))
    .filter(({ row }) => localRowNeedsRemoteSave(row) || localRowHasLaptopOnlyImages(row));

  if (!candidateIndexes.length) {
    if (manual) setSyncStatus("No laptop-only data found.", "success");
    return 0;
  }
  ...
  try {
    for (const { row, index } of candidateIndexes) {
      const existingBefore = imagePathFromValue(localPortableImage(row, "before")) || localPortableImage(row, "before");
      const existingAfter = imagePathFromValue(localPortableImage(row, "after")) || localPortableImage(row, "after");
      const beforeImage = existingBefore || await uploadDataImage(localDataImage(row, "before"), row, "before");
      const afterImage = existingAfter || await uploadDataImage(localDataImage(row, "after"), row, "after");

      if (!localRowNeedsRemoteSave(row)) {
        localRows[index] = cleanedSyncedLocalRow(row, null, beforeImage, afterImage);
        continue;
      }

      setSyncStatus(`Syncing ${syncedCount + 1} of ${candidateIndexes.length}...`);

      if (isUuid(row.id)) {
        const { data, error } = await window.sb
          .from("trades")
          .update({ before_url: beforeImage || null, after_url: afterImage || null })
          .eq("id", row.id)
          .select()
          .single();

        if (error) throw error;
        localRows[index] = cleanedSyncedLocalRow(row, data, beforeImage, afterImage);
      } else {
        const dbTrade = buildSupabaseTrade(row, beforeImage, afterImage);
        const { data, error } = await window.sb
          .from("trades")
          .insert([dbTrade])
          .select()
      ...
      syncedCount += 1;
      saveLocalTrades(localRows);
    }

    saveLocalTrades(localRows);
    if (syncedCount) setSyncStatus(`Synced ${syncedCount} local trade${syncedCount === 1 ? "" : "s"}.`, "success");
    else if (manual) setSyncStatus("Local cache cleaned.", "success");
    return syncedCount;
  } catch (error) {
    console.warn("Local Supabase sync failed:", error.message);
    setSyncStatus(error.message || "Local sync failed.", "warning");
    saveLocalTrades(localRows);
    return syncedCount;
  } finally {
...
async function applyTrades(remoteRows = remoteRowsCache, { hydrateImages = false } = {}) {
  const localRows = readLocalTrades();
  const importedRows = readImportedTrades();
  allTrades = mergeTrades(localRows, remoteRows, importedRows);
  if (hydrateImages) allTrades = await hydrateTradeImages(allTrades);
  populatePairFilter();
...
async function init() {
  ...
  let remoteRows = await fetchRemoteTrades();
  if (currentLoad !== loadToken) return;

  const syncedCount = await syncLocalTradesToSupabase();
  if (currentLoad !== loadToken) return;
  if (syncedCount) remoteRows = await fetchRemoteTrades();

  remoteRowsCache = remoteRows;
  await applyTrades(remoteRowsCache, { hydrateImages: true });
}
...
syncLocalBtn?.addEventListener("click", async () => {
  const syncedCount = await syncLocalTradesToSupabase({ manual: true });
  if (syncedCount) {
    remoteRowsCache = await fetchRemoteTrades();
    await applyTrades(remoteRowsCache, { hydrateImages: true });
  }
});
